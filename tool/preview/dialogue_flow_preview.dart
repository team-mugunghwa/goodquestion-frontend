...
class _Scene {
  const _Scene(this.order, this.type, this.narration,
      {this.character, this.opening, this.elements = const <String>[], this.resultImage});
  ...
  PlayScene toEntity() => PlayScene(
    ...
    sceneOrder: order,
    sceneType: type,
    narrationSentences: type == PlaySceneType.story ? <String>[narration] : const <String>[],
...
const List<_Scene> _script = <_Scene>[
  ...
  _Scene(3, PlaySceneType.dialogue, '', character: '방귀쟁이 며느리',
  ...
  _Scene(5, PlaySceneType.dialogue, '', character: '시아버지',
  ...
  _Scene(7, PlaySceneType.dialogue, '', character: '마을 이장',
      opening: '이 배나무는 너무 높아서 아무도 딸 수가 없었단다. 무슨 뾰족한 방법이 없겠는가?',
      elements: <String>['SOLUTION', 'REASON', 'REQUEST', 'RESULT'],
      resultImage: 'assets/images/dialogue/banggui/scene_07/scene_background_blurred.webp'),
  _Scene(8, PlaySceneType.story, '시아버지는 며느리의 방귀가 특별한 힘이라는 것을 깨닫고 사과했습니다.'),
  _Scene(9, PlaySceneType.dialogue, '', character: '방귀쟁이 며느리',
...
class _FlowRepository implements PlayRepository {
  ...
  Future<PlaySessionSnapshot> resume(String sessionId) async {
    if (_postActivity) {
      onLog('후속 활동으로 이동');
      return const PlaySessionSnapshot(phase: PlayPhase.postActivity, currentScene: null);
    }
    return _snapshot();
  }
  ...
  Future<PlayTurnResult> submitUtterance(
    ...
    // 1턴에 절반, 2턴에 나머지를 채운다.
    _accumulated.addAll(_turn == 1
        ? elements.take((elements.length / 2).ceil())
        : elements);
    final bool closing = _turn >= 2;
    onLog('  턴 $_turn · 누적 ${_accumulated.join(",")}${closing ? " · 장면 종료" : ""}');
    ...
    if (!closing) {
      return PlayTurnResult(
        ...
        analysis: PlayAnalysis(
            detectedElements: _accumulated.toList(), utteranceValidity: 'VALID'),
    ...
    final PlayTurnResult result = PlayTurnResult(
      ...
      sceneTransition: PlaySceneTransition(
        next: isLast ? PlayTransitionTarget.postActivity : PlayTransitionTarget.scene,
  ...
  @override
  Future<PlayOpeningMessage> openCurrentScene(String sessionId) async => PlayOpeningMessage(
    text: _scene.opening ?? '',
    audioUrl: null,
    alreadyOpened: true,
  );
...
class _FlowPreviewAppState extends State<FlowPreviewApp> {
  ...
  late final GoRouter _router = GoRouter(
    ...
    routes: <RouteBase>[
      ...
      GoRoute(path: AppRoutes.playRecapPath, builder: (_, _) => const _EndPage('후속 활동')),
