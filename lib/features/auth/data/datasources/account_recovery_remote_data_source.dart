...
class AccountRecoveryRemoteDataSource {
  ...
  ///
  /// [childName] 과 [childAge] 는 스키마상 선택이지만 **둘 다 채워야** 그
  /// 아이로 좁혀서 찾습니다. 하나라도 비면 서버가 보호자 이름만으로 찾고,
  /// 그때는 **아이가 등록된 계정을 결과에서 뺍니다** - 아이 정보 없이는 남의
  /// 계정을 캐낼 수 없게 막는 조건이라, 실사용자는 사실상 늘 빈 결과를 받습니다.
  ///
  /// **나이로 받아 출생연도로 바꿔 보냅니다.** 아이 등록(`createChild`)이
  /// 나이를 받아 같은 식으로 저장하므로, 여기서만 다른 값을 받으면 보호자가
  /// 두 화면에서 다른 것을 다루게 되고 변환이 어긋나 못 찾습니다.
  /// → [birthYearFromAge]
  ///
  ...
  Future<List<String>> findEmails({
    ...
    body: <String, dynamic>{
      ...
      'childName': childName,
      'childBirthYear': childAge == null ? null : birthYearFromAge(childAge),
    },
    parse: (Object? data) {
