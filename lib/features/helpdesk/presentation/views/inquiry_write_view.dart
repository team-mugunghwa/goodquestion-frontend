...
class InquiryWritePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider<InquiryWriteViewModel>(
      create: (_) => InquiryWriteViewModel(getIt<CreateInquiryUseCase>()),
      child: const InquiryWriteView(),
    );
  }
}

class InquiryWriteView extends StatefulWidget {
  const InquiryWriteView({super.key});
  ...
  Future<void> _submit() async {
    ...
    if (created == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(vm.errorMessage ?? '문의를 등록하지 못했습니다.')),
      );
      return;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('문의를 접수했습니다. 답변이 등록되면 알려드릴게요.')),
    );
    context.pop();
  }
        ...
        child: ListView(
          ...
          children: <Widget>[
            ...
            FilledButton(
              ...
              child: Text(vm.isSubmitting ? '등록 중...' : '문의 등록'),
            ),
          ],
        ),
      ),
